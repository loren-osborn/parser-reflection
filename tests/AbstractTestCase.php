...
abstract class AbstractTestCase extends \PHPUnit_Framework_TestCase
{
    ...
    protected function getStringificationOf($value, $maxLen = 128)
    {
        ...
        if (gettype($value) == 'string') {
            ...
            $cropCount = 3 + mb_strlen($result) - $maxLen;
            return var_export(
                mb_substr($value, 0, max(0, mb_strlen($value) - $cropCount)) . '...',
                true);
        }
        if (gettype($value) == 'array') {
            ...
            $minContentLen = 0;
            $processPart = (function ($idx, $type, $partVal) use (&$stringifiedParts, &$stringifiedPartsByLength, &$minContentLen, $minElementsToDisplay, $contentCharCount) {
                $shortest = $this->getStringificationOf($partVal, 0);
                $longest = $this->getStringificationOf($partVal, $contentCharCount + 1);
                if (mb_strlen($shortest) > mb_strlen($longest)) {
                    $shortest = $longest;
                }
                if ($idx < $minElementsToDisplay) {
                    $minContentLen += mb_strlen($shortest);
                }
                $stringifiedParts["{$type}_{$idx}"] = [
                    ...
                    'shortStr'        => $shortest,
                    'longStr'         => $longest,
                    'minLen'          => mb_strlen($shortest),
                    'maxLen'          => mb_strlen($longest),
                    'needsTruncation' => (mb_strlen($longest) > $contentCharCount),
                    'alwaysInclude'   => ($idx < $minElementsToDisplay),
                ];
                if (!isset($stringifiedPartsByLength[mb_strlen($longest)])) {
                    $stringifiedPartsByLength[mb_strlen($longest)] = [];
                }
                $stringifiedPartsByLength[mb_strlen($longest)][] = "{$type}_{$idx} longest";
                if (!isset($stringifiedPartsByLength[mb_strlen($shortest)])) {
                    $stringifiedPartsByLength[mb_strlen($shortest)] = [];
                }
                $stringifiedPartsByLength[mb_strlen($shortest)][] = "{$type}_{$idx} shortest";
            });
                ...
                foreach ($partIdList as $partIdAndMode) {
                    ...
                    if (!isset($stringifiedParts["{$type}_{$idx}"]['finalString'])) {
                        ...
                        if (
                            ...
                            $contentCharsLeft -= mb_strlen($finalStr);
                            $partsLeft -= 1;
                        }
                    }
                }
            ...
            do {
                ...
                (
                    !$overflow ||
                    (mb_strlen("{$result}{$lastItem}{$arraySuffix}") < $maxLen)
                )
            );
            if (!$overflow) {
                $result .= $lastItem;
        ...
        if (mb_strlen(get_class($value)) > $maxLen) {
            return 'object';
        }
        if (mb_strlen(get_class($value).'Object()') > $maxLen) {
            return get_class($value);
        }
        if (mb_strlen(get_class($value).'Object()()' . spl_object_hash($value)) > $maxLen) {
            return 'Object(' . get_class($value) . ')';
        }
        if (($this->isConvertableToString($value))) {
            $asStr = (string)$value;
            $maxAsStrLen = $maxLen - mb_strlen(get_class($value).'Object()(): ' . spl_object_hash($value));
            $result = sprintf(
                'Object(%s(%s: %s))',
                get_class($value),
    ...
    protected function assertRefelctorValueSame($expected, $actual, $message = '')
    {
        if (!is_object($expected) && !is_array($expected)) {
            ...
            foreach (array_keys($expected) as $exIndex => $exKey) {
                ...
                $this->assertRefelctorValueSame($expected[$exKey], $actual[$exKey], $message);
            }
        }
