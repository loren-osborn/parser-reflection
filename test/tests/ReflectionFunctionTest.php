...
class ReflectionFunctionTest extends TestCaseBase
{
    ...
    {
        $files = ['PHP5.5' => ['fileName' => $this->getStubDir() . '/FileWithFunctions55.php']];

        if (PHP_VERSION_ID >= 70000) {
            $files['PHP7.0'] = ['fileName' => $this->getStubDir() . '/FileWithFunctions70.php'];
        }
    ...
    public function functionProvider()
    {
        ...
        foreach ($files as $filenameArgList) {
            ...
            if (!$fileName) {
                throw new \Exception(sprintf('bad filename in %s', var_export($files, true)));
            }
            $resolvedFileName = stream_resolve_include_path($fileName);
            if (!$resolvedFileName) {
                throw new \Exception(sprintf('filename %s didn\'t resolve', var_export($fileName, true)));
            }
