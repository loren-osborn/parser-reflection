...
class TestCaseBase extends PHPUnit_Framework_TestCase
{
    ...
    {
        return dirname(__DIR__) . '/support/Stubs';
    }

    public function getStubNamespace()
    {
        return 'Go\\ParserReflection\\TestingSupport\\Stubs';
    }

    public function getStubNamespaceFromFilename($fileName)
    {
        $stubNamespace          = $this->getStubNamespace();
        $stubRightmostNamespace = preg_replace('/^.*\\\\/', '', trim($stubNamespace, '\\')); 
        // $stubRightmostNamespace == 'Stubs'
        ...
        // Remove everything before $stubRightmostNamespace.
        while (count($filenameParts) && (reset($filenameParts) !== $stubRightmostNamespace)) {
            array_shift($filenameParts);
        }
        // Remove $stubRightmostNamespace.
        array_shift($filenameParts);

        // Prepend namespace prefix.
        return trim($stubNamespace . '\\' . implode('\\', $filenameParts), '\\');
    }
